    ...
    if($_FILES ['foto']["error"]> 0){
        ...
        if(in_array($_FILES['foto']['type'], $PERMITIDOS) && $_FILES['foto']['size'] <= LIMITE * 1024){
            ...
            $resultado = move_uploaded_file($_FILES['foto']['tmp_name'], $foto);
                ...
                // validamos que las variables no esten vacias
                if (strlen($tipodocumento)>0 && strlen($identificacion)>0 && strlen($nombres)>0 && strlen($apellidos)>0 && strlen($fecha_nacimiento)>0 && strlen($direccion)>0 && strlen($ciudad)>0 && strlen($telefono)>0 && strlen($email)>0) {

                    // encriptamos la clave con la instruccion md5
                  
                    $objetoConsultas = new consultasUsuario();

                    $result = $objetoConsultas->modificarUser($tipodocumento, $identificacion, $nombres, $apellidos, $fecha_nacimiento, $direccion, $ciudad, $telefono, $email, $foto);
            ...
            // si los campos vienen vacios redireccionamos al formulario
                ...
                echo '<script> location.href="../view/Usuario/editarUsuario.php"</script>';
            }
