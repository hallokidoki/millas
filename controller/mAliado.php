    ...
    if($_FILES ['foto']["error"]> 0){
        ...
        if(in_array($_FILES['foto']['type'], $PERMITIDOS) && $_FILES['foto']['size'] <= LIMITE * 1024){
            ...
            $resultado = move_uploaded_file($_FILES['foto']['tmp_name'], $foto);
                ...
                // validamos que las variables no esten vacias
                if (strlen($tipodocumento)>0 && strlen($identificacion)>0 && strlen($nombres)>0 && strlen($direccion)>0 && strlen($ciudad)>0 && strlen($telefono)>0 && strlen($email)>0 ) {
                    // encriptamos la clave con la instruccion md5
                  
                    $objetoConsultas = new consultasAliados();

                    $result = $objetoConsultas->modificarAliado($tipodocumento, $identificacion, $nombres, $direccion, $ciudad, $telefono, $email,$foto);
            // si los campos vienen vacios redireccionamos al formulario
            }else {
                
                echo "<script> alert ('PORFAVOR COMPLETE LOS CAMPOS')</script>";
                echo '<script> location.href="../view/Aliados/editarAliado.php"</script>';
            }
